  ...
  <div class="app-container">
    ...
    <el-table
      ...
      <el-table-column label="状态" width="80">
        <template slot-scope="scope">
          <el-switch
            v-model="scope.row.status === 1" :disabled="false" @change="switchStatus(scope.row)">
          </el-switch>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    //添加
    save() {
      api.addMenu(this.sysMenu).then(response => {
    ...
    //更新
    update() {
      api.updateMenu(this.sysMenu).then(response => {
        this.$message.success(response.message || '操作成功')
        this.dialogVisible = false
        this.fetchData()
      })
